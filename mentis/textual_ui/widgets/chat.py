...
from mentis.config import AppConfig
from mentis.runtime import (
    ConversationManager,
    Conversation,
    StreamingMessage,
    AppContext,
)
from mentis.runtime.langchain.schema import new_message_of_type

from mentis.textual_ui.widgets.typing_indicator import IsTyping
from mentis.textual_ui.widgets.header import ChatHeader
from mentis.textual_ui.widgets.model_select import ModelSelect
from mentis.textual_ui.widgets.chatbox import Chatbox, ChatboxContainer
...
class Chat(Widget):
    ...
    def update_header(self, *, title=None, model_name=None, api_provider=None):
        ...
        self.log.debug(f"chat header info: {chat.model.name}@{chat.model.api_provider}")
    ...
    async def progress_conversation(self, message: BaseMessage) -> None:
        ...
        if new_chat:
            ...
            self.update_header(
                title=Conversation.preview_from_messages([self.chat_id]),
            )
    ...
    async def _stream_llm_response(
        ...
        async for resp in conversation.progress(message):
            response = resp
            self.log(f"!!!!!!Got response: {response}")
            ...
            if isinstance(response, BaseMessage):
                ...
                """
                self.log(f"@@@@{response_chatbox.message.additional_kwargs}")
